                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <input onChange={(e) => {
                                    const updatedtFiles = [...files]
                                    updatedtFiles[index] = e.target.files[0]
                                    setFiles(updatedtFiles)
                                }} type="file" id={`image${index}`} hidden />

                              <img  className="cursor-pointer" src={files[index]?URL.createObjectURL(files[index]):assets.upload_area} alt="uploadArea" width={80} height={80} />
                            </label>
                        ))}
                    </div>
                </div>
